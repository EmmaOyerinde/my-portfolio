...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function createBasemaps() {
        return {
            // ENHANCEMENT: Swapped CARTO to Esri Canvas Maps (100% free, no API key required, removes watermark bug)
    ...
    // ENHANCEMENT: includeGeocoder flag prevents duplicating search bars on lower maps.
    ...
    function initReliabilityMap(utilitiesGeoJSON, canadaBounds) {
        ...
        // ENHANCEMENT: Force size recalculation when scrolled into view
        ...
        // ENHANCEMENT: True adds the geocoder search bar to THIS top map only
    ...
    function initForecastMap(regions, canadaBounds) {
        ...
        // ENHANCEMENT: Force size recalculation when scrolled into view
        const mapObserver = new IntersectionObserver(entries => {
            if(entries[0].isIntersecting) { forecastMap.invalidateSize(); }
        });
        mapObserver.observe(mapEl);
        
        // ENHANCEMENT: False prevents geocoder duplication
    ...
    function initDeficitMap(gridData, canadaBounds) {
        ...
        // ENHANCEMENT: Force size recalculation when scrolled into view
        const mapObserver = new IntersectionObserver(entries => {
            if(entries[0].isIntersecting) { deficitMap.invalidateSize(); }
        });
        mapObserver.observe(mapEl);
    ...
    function initOffGridMap(offgridZones, directoryData, canadaBounds) {
        ...
        // ENHANCEMENT: Force size recalculation when scrolled into view
        const mapObserver = new IntersectionObserver(entries => {
            if(entries[0].isIntersecting) { offgridMap.invalidateSize(); }
        });
        mapObserver.observe(mapEl);
